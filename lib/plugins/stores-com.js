const productionSites = '(?:https?:\\/\\/(?:www\\.)?)?(?<site>(?:casemates|hammacher|meh|morningsave|shop\\.univision|sidedeal)\\.com)?';
        ...
        if (hrefIndex !== -1) {
            ...
            if (href) {
                ...
                if (textToken?.content && regExp.test(textToken.content)) {
                    ...
                    let embedUrl = `/${matches.groups.path}/embed`;

                    const site = matches.groups.site || matches.groups.siteLocal;
                    if (site) {
