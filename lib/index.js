...
const commands = {
    ...
    emojify: function(args, callback) {
        ...
        const tokens = new natural.WordTokenizer().tokenize(args);
        ...
        tokens.forEach(t => {
            ...
            const whitelist = emojiMappings.whitelist[t.toLowerCase()];
            ...
            const stem = natural.PorterStemmer.stem(t.toLowerCase());

            // Check to see if the word directly matches an Emoji shortname
            for (const key in emoji) {
                const stemmedShortname = natural.PorterStemmer.stem(key.toLowerCase());
    ...
    google: function(args, callback) {
        ...
        request.get(`https://www.googleapis.com/customsearch/v1?cx=001016559301879871327:usuoorwmjkw&key=${process.env.GOOGLE_API_KEY}&q=${encodeURIComponent(args)}&safe=high`, { json: true }, function(err, res, body) {
            ...
            if (!body || !body.items || !body.items.length || !body.items[0] || !body.items[0].link) {
                //eslint-disable-next-line no-console
                console.warn(`/google no results: ${res?.statusCode} ${JSON.stringify(body?.error || body?.searchInformation || 'no body')}`);
                return callback();
            }
    ...
    image: function(args, callback) {
        ...
        request.get(`https://www.googleapis.com/customsearch/v1?cx=001016559301879871327:usuoorwmjkw&key=${process.env.GOOGLE_API_KEY}&q=${encodeURIComponent(args)}&safe=high&searchType=image`, { json: true }, function(err, res, body) {
            ...
            if (!body || !body.items || !body.items.length || !body.items[0] || !body.items[0].link) {
                //eslint-disable-next-line no-console
                console.warn(`/image no results: ${res?.statusCode} ${JSON.stringify(body?.error || body?.searchInformation || 'no body')}`);
                return callback();
            }
...
exports.kalturaEmbedHtml = function(urlStr) {
    ...
    const partner_id = matches[1];
    const uiconf_id = matches[2];
    const entry_id = matches[3];

    const embedUrl = `https://cdnapisec.kaltura.com/p/${partner_id}/embedIframeJs/uiconf_id/${uiconf_id}?iframeembed=true&entry_id=${entry_id}`;
...
exports.render = function(markdown, options, callback) {
    ...
    if (callback) {
        executor().then(result => callback(null, result)).catch(err => callback(err));
    } else {
        return executor();
    }
};
...
exports.youTubeEmbedHtml = function(urlStr) {
    ...
    const matches = uri.href.match(/(?:https?:\/\/(?:www\.)?)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&(?:amp;)?v=))((\w|-){11})(?:\S+)?/i);

    if (!matches || !matches[1]) {
        return '';
    }

    let embedUrl = `https://www.youtube.com/embed/${matches[1]}?autohide=1&color=white&showinfo=0&theme=light`;
