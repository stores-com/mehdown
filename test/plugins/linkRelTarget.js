...
test('linksRelTarget', { concurrency: true }, async (t) => {
    await t.test('should not set rel attribute or target attribute if there is no baseUrl specified', async () => {
        const html = await render('[Google](https://www.google.com)');
        assert.strictEqual(html, '<p><a href="https://www.google.com">Google</a></p>');
    });

    await t.test('should set rel attribute to "nofollow" and target attribute to "_blank" for a URL that is not at the same domain as the base URL', async () => {
    ...
    await t.test('should set rel attribute to "nofollow" and target attribute to "_blank" for a scheme relative URL that is not at the same domain as the base URL', async () => {
    ...
    await t.test('should not set rel attribute or target attribute for a local URL', async () => {
        const html = await render('[path](/path)', { baseUrl: 'https://mediocre.com' });
        assert.strictEqual(html, '<p><a href="/path">path</a></p>');
    });

    await t.test('should not set rel attribute to "nofollow" and target attribute to "_blank" for a baseUrl with an IP address and port', async () => {
    ...
    await t.test('should not set rel attribute to "nofollow" and target attribute to "_blank" for a URL that is at the same domain as the base URL', async () => {
