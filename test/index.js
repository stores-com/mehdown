...
test('bbcode', { concurrency: true }, async (t) => {
    ...
    await t.test('[quote=Bjarne]', async () => {
    ...
    await t.test('[url] with text', async () => {
...
test('commands', async (t) => {
    await t.test('/coinflip', { concurrency: true }, async (t) => {
        await t.test('/coinflip', async () => {
    ...
    await t.test('/emojify', { concurrency: true }, async (t) => {
        await t.test('/emojify', async () => {
            const html = await render('/emojify');
            assert.strictEqual(html, '<p>/emojify</p>');
        });

        await t.test('/emojify Basketball finishes at 5. Then it\'s pizza or tacos. Maybe go to the movies. You in?', async () => {
        ...
        await t.test('/emojify package package :package: package', async () => {
        ...
        await t.test('/emojify no woman, no cry on mail so', async () => {
        ...
        await t.test('/emojify shit', async () => {
    ...
    await t.test('/google', { timeout: 10000 }, async (t) => {
        ...
        await t.test('/google meh with API key', { skip: !process.env.GOOGLE_API_KEY }, async () => {
            const html = await render('/google meh');
            assert.notStrictEqual(html, '<p>/google meh</p>');
        });
    });

    await t.test('/image', { timeout: 10000 }, async (t) => {
        await t.test('/image', async () => {
            const html = await render('/image');
            assert.strictEqual(html, '<p>/image</p>');
        });

        await t.test('/image meh', { skip: !process.env.GOOGLE_API_KEY }, async () => {
            const html = await render('/image meh');
            assert.notStrictEqual(html, '<p>/image meh</p>');
        });

        await t.test('/image neon pink on black', { skip: !process.env.GOOGLE_API_KEY }, async () => {
            const html = await render('/image neon pink on black');
            assert.notStrictEqual(html, '<p>/image neon pink on black</p>');
        });

        await t.test('/image THISISSOMETEXTTHATGOOGLEDOESNOTUNDERSTAND', { skip: !process.env.GOOGLE_API_KEY }, async () => {
    ...
    await t.test('/jumble', { concurrency: true }, async (t) => {
        await t.test('/jumble', async () => {
            const html = await render('/jumble');
            assert.strictEqual(html, '<p>/jumble</p>');
        });

        await t.test('/jumble text', async () => {
    ...
    await t.test('/piglatin', { concurrency: true }, async (t) => {
        await t.test('/piglatin', async () => {
            const html = await render('/piglatin');
            assert.strictEqual(html, '<p>/piglatin</p>');
        });

        await t.test('/piglatin text', async () => {
    ...
    await t.test('/roll', { concurrency: true }, async (t) => {
        ...
        await t.test('/roll 2d20 --show', async () => {
        ...
        await t.test('/roll 5d10+5', async () => {
    ...
    await t.test('/rot13', { concurrency: true }, async (t) => {
        await t.test('/rot13', async () => {
            const html = await render('/rot13');
            assert.strictEqual(html, '<p>/rot13</p>');
        });

        await t.test('/rot13 text', async () => {
    ...
    await t.test('/shrug', { concurrency: true }, async (t) => {
        await t.test('/shrug', async () => {
    ...
    await t.test('/youtube', { timeout: 10000 }, async (t) => {
        await t.test('/youtube', async () => {
            const html = await render('/youtube');
            assert.strictEqual(html, '<p>/youtube</p>');
        });

        await t.test('/youtube meh without API key', async () => {
            const googleApiKey = process.env.GOOGLE_API_KEY;
            process.env.GOOGLE_API_KEY = undefined;
        ...
        await t.test('/youtube Purple Reign', { skip: !process.env.GOOGLE_API_KEY }, async () => {
            const html = await render('/youtube Purple Reign');
            assert.notStrictEqual(html, '<p>/youtube Purple Reign</p>');
        });

        await t.test('/youtube simon\'s cat', { skip: !process.env.GOOGLE_API_KEY }, async () => {
...
test('detect image sizes', async (t) => {
    ...
    await t.test('broken image', { timeout: 10000 }, async () => {
    ...
    await t.test('broken image html', async () => {
        const html = await render('http://google.com/404.png', { detectImageSizes: true });
        assert.strictEqual(html, '<p><img src="http://google.com/404.png" /></p>');
    });

    await t.test('scheme relative broken image html', async () => {
        const html = await render('//meh.com/404.png', { detectImageSizes: true });
        assert.strictEqual(html, '<p><img src="//meh.com/404.png" /></p>');
    });

    await t.test('image from editor', async () => {
    ...
    await t.test('scheme relative image html', async () => {
    ...
    await t.test('BUG: Cannot read property "height" of undefined', async () => {
...
test('email', { concurrency: true }, async (t) => {
    await t.test('email address', async () => {
    ...
    await t.test('email addresses with periods should be linked correctly', async () => {
...
test('html', { concurrency: true }, async (t) => {
    await t.test('convertRelativeUrlsToAbsoluteUrls', { concurrency: true }, async (t) => {
        await t.test('should convert relative images', () => {
        ...
        await t.test('should convert relative images with slash at the beginning', () => {
            const html = '<img src="/foo.png">';
            const converted = mehdown.html.convertRelativeUrlsToAbsoluteUrls(html, 'http://mysite.com');
            assert.strictEqual(converted, '<img src="http://mysite.com/foo.png">');
        });

        await t.test('should convert relative scripts with slash at the beginning', () => {
            const html = '<script src="/test.js"></script>';
            const converted = mehdown.html.convertRelativeUrlsToAbsoluteUrls(html, 'http://mysite.com');
            assert.strictEqual(converted, '<script src="http://mysite.com/test.js"></script>');
        });

        await t.test('should convert relative images with slash at the beginning and trailing slash for base url', () => {
            const html = '<img src="/foo.png">';
            const converted = mehdown.html.convertRelativeUrlsToAbsoluteUrls(html, 'http://mysite.com/');
            assert.strictEqual(converted, '<img src="http://mysite.com/foo.png">');
        });

        await t.test('should convert relative links', () => {
            const html = '<a href="mypage"></a>';
            const converted = mehdown.html.convertRelativeUrlsToAbsoluteUrls(html, 'http://mysite.com');
            assert.strictEqual(converted, '<a href="http://mysite.com/mypage"></a>');
        });

        await t.test('should not convert absolute links', () => {
            const html = '<a href="http://google.com">test</a>';
            const converted = mehdown.html.convertRelativeUrlsToAbsoluteUrls(html, 'http://mysite.com');
            assert.strictEqual(converted, '<a href="http://google.com">test</a>');
        });

        await t.test('should not fail with links without href', () => {
            const html = '<a>test</a>';
            const converted = mehdown.html.convertRelativeUrlsToAbsoluteUrls(html, 'http://mysite.com');
    ...
    await t.test('convertToLazyLoadedImages', { concurrency: true }, async (t) => {
        await t.test('no images', () => {
    ...
    await t.test('permalinkHeaders', { concurrency: true }, async (t) => {
        await t.test('no headers', () => {
            const html = '<div><span>hello</span> <span>world</span></div>';
            const html2 = mehdown.html.permalinkHeaders(html);
            assert.strictEqual(html, html2);
        });

        await t.test('one header without id', () => {
            const html = '<div><h1>hello world</h1><p>header</p></div>';
            const html2 = mehdown.html.permalinkHeaders(html);
            assert.strictEqual(html, html2);
        });

        await t.test('one header', () => {
    ...
    await t.test('removeAttribute', { concurrency: true }, async (t) => {
        await t.test('removeAttribute()', () => {
            assert.strictEqual(mehdown.html.removeAttribute(), undefined);
        });

        await t.test('removeAttribute(html, attribute) no match', () => {
        ...
        await t.test('removeAttribute(html, attribute) match', () => {
            const html = '<div class="foo"><span>hello</span> <span>world</span></div>';
            assert.strictEqual(mehdown.html.removeAttribute(html, 'class'), '<div ><span>hello</span> <span>world</span></div>');
        });
    });

    await t.test('removeImageSizeAttributes', { concurrency: true }, async (t) => {
        await t.test('removeImageSizeAttributes(htmlWithoutImage)', () => {
            const html = '<div><span>hello</span> <span>world</span></div>';
            const html2 = mehdown.html.removeImageSizeAttributes(html);
            assert.strictEqual(html, html2);
        });

        await t.test('removeImageSizeAttributes(imagesWithoutSizes)', () => {
        ...
        await t.test('removeImageSizeAttributes(htmlWithImages)', () => {
...
test('links', { concurrency: true }, async (t) => {
    ...
    await t.test('[see http://example.com here](http://example.com)', async () => {
    ...
    await t.test('[leavemealone.com](http://leavemealone.com) but linkme.com', async () => {
    ...
    await t.test('[example.com](http://example.com "go to example.com")', async () => {
    ...
    await t.test('simple domain', async () => {
    ...
    await t.test('domain with path', async () => {
    ...
    await t.test('domain with query string', async () => {
    ...
    await t.test('URLs with underscores should not lose the underscores', async () => {
    ...
    await t.test('should not link two, four, or five consecutive periods', async () => {
    ...
    await t.test('should link URLs with @ characters', async () => {
    ...
    await t.test('URLs that happen to have Emoji shortnames should be linked correctly', async () => {
    ...
    await t.test('anchor with blank href', async () => {
...
test('newlines', { concurrency: true }, async (t) => {
    ...
    await t.test('<blockquote> long', async () => {
...
test('mehdown.youTubeEmbedHtml', { concurrency: true }, async (t) => {
    await t.test('http://www.youtube.com/watch?v=kU9MuM4lP18', () => {
    ...
    await t.test('http://www.youtube.com/watch?feature=player_embedded&v=zIEIvi2MuEk', () => {
    ...
    await t.test('`&amp;` instead of `&` in URL', () => {
        const html = mehdown.youTubeEmbedHtml('http://www.youtube.com/watch?feature=player_embedded&amp;v=zIEIvi2MuEk');
        assert.strictEqual(html, '<iframe allowfullscreen class="youtube" frameborder="0" src="https://www.youtube.com/embed/zIEIvi2MuEk?autohide=1&color=white&showinfo=0&theme=light"></iframe>');
    });

    await t.test('http://www.youtube.com/watch?v=kU9MuM4lP18&start=10', () => {
    ...
    await t.test('http://www.youtube.com/watch?v=kU9MuM4lP18&start=10&end=20', () => {
